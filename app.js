...
const translations = {
  ja: {
    ...
    aiSummarySection: "AIリサーチ要約",
    aiSummaryDemoLabel: "事前生成デモコンテンツ",
    aiSummaryTitleText: "投票前のリサーチ論点",
    aiSummaryOverview:
      "このサマリーはライブAI生成ではありません。Track 2向けの将来ワークフローを示すため、過去サンプルデータから事前に作成したデモ要約です。",
    aiSummaryPointLiquidity:
      "流動性：Aave預入の増加は、Mantle上の資金流入と利用意欲を確認する重要な論点です。",
    aiSummaryPointRwa:
      "RWA：トークン化資産の活動が強まる場合、DeFi以外の需要拡大として評価できます。",
    aiSummaryPointNarrative:
      "AI / InfoFi：AIや予測市場の注目度は、短期的な市場心理とエコシステム関心を測る補助シグナルです。",
    aiSummaryCaution:
      "この内容は投票判断を整理するためのデモ要約であり、金融助言ではありません。ライブAI生成は今後のロードマップです。",
    ...
    architectureItems: [
      ...
      "demoVotes",
      "demoUserHistory",
      "aiSummaryProvider pre-generated demo",
  ...
  en: {
    ...
    aiSummarySection: "AI Research Summary",
    aiSummaryDemoLabel: "Pre-generated demo content",
    aiSummaryTitleText: "Research points before voting",
    aiSummaryOverview:
      "This summary is not generated by live AI. It is pre-generated demo content from historical sample data to show the future Track 2 research workflow.",
    aiSummaryPointLiquidity:
      "Liquidity: Higher Aave deposits are a key point for assessing capital inflow and user intent on Mantle.",
    aiSummaryPointRwa:
      "RWA: Stronger tokenized asset activity can indicate demand expanding beyond DeFi-native use cases.",
    aiSummaryPointNarrative:
      "AI / InfoFi: Attention around AI and prediction markets is a supporting signal for short-term sentiment and ecosystem interest.",
    aiSummaryCaution:
      "This is a demo summary to help organize voting context, not financial advice. Live AI generation is reserved for the roadmap.",
    ...
    roadmapItems: [
      ...
      "Live AI research summaries from Mantle ecosystem data",
...
function renderAiSummary(round) {
  ...
  return `
    <section class="ai-summary-panel">
      <div class="section-heading">
        <div>
          <h2>${t("aiSummarySection")}</h2>
          <p>${t(summary.overviewKey)}</p>
        </div>
        <span>${t(summary.labelKey)}</span>
      </div>
      <h3>${t(summary.titleKey)}</h3>
      <ul class="ai-summary-list">
        ${summary.points.map((pointKey) => `<li>${t(pointKey)}</li>`).join("")}
      </ul>
      <p class="ai-summary-caution">${t(summary.cautionKey)}</p>
    </section>
  `;
}
...
void aiSummaryProvider.getSummary(demoRounds.find((round) => round.status === "open"));
